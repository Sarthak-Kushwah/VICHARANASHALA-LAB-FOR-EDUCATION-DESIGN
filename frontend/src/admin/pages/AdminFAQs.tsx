...
interface FAQ { _id: string; question: string; answer: string; category: string; status: 'approved' | 'pending' | 'rejected'; views: number; helpfulVotes: number; createdAt: string; }
...
export default function AdminFAQs() {
  ...
  const handleAdd = async () => {
    setSaving(true);
    try { await adminApi.post('/admin/faq', newFaq); showToast('Created'); setAddModal(false); setNewFaq({ question: '', answer: '', category: '', status: 'approved' }); fetchFaqs(); }
  ...
  return (
    <div className="space-y-4 max-w-6xl">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)} className="px-2.5 py-1.5 rounded-md text-sm text-gray-700 bg-white border border-gray-200 outline-none focus:border-gray-400">
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? <tr><td colSpan={7} className="px-3 py-6"><TableSkeleton rows={8} /></td></tr> :
               faqs.length === 0 ? <tr><td colSpan={7} className="px-3 py-12 text-center text-sm text-gray-400">No FAQs found</td></tr> :
               faqs.map(faq => (
                <tr key={faq._id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
                  <td className={`${td} max-w-[220px] truncate`} title={faq.question}>{faq.question}</td>
                  <td className={`${td} text-gray-500`}>{faq.category}</td>
                  <td className={td}><Badge status={faq.status as 'approved'|'pending'|'rejected'} /></td>
                  <td className={`${td} text-right tabular-nums text-gray-500`}>{faq.views ?? 0}</td>
                  <td className={`${td} text-right tabular-nums text-gray-500`}>{faq.helpfulVotes ?? 0}</td>
                  <td className={`${td} text-gray-500`}>{new Date(faq.createdAt).toLocaleDateString('en-IN')}</td>
                  <td className={`${td} text-right`}>
                    <div className="flex items-center justify-end gap-1">
                      {faq.status !== 'approved' && <button onClick={() => handleApprove(faq._id)} className="w-6 h-6 flex items-center justify-center rounded text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 transition-colors" title="Approve"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg></button>}
                      {faq.status !== 'rejected' && <button onClick={() => handleReject(faq._id)} className="w-6 h-6 flex items-center justify-center rounded text-gray-400 hover:text-amber-600 hover:bg-amber-50 transition-colors" title="Reject"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>}
                      <button onClick={() => { setEditFaq({ ...faq }); setEditModal(true); }} className="w-6 h-6 flex items-center justify-center rounded text-gray-400 hover:text-gray-800 hover:bg-gray-100 transition-colors" title="Edit"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
                      <button onClick={() => handleDelete(faq._id)} className="w-6 h-6 flex items-center justify-center rounded text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors" title="Delete"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        {pages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 text-sm text-gray-500">
            ...
            <div className="flex gap-1">
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="px-3 py-1 rounded-md hover:bg-gray-100 disabled:opacity-30 transition-colors">← Prev</button>
              <button onClick={() => setPage(p => Math.min(pages, p + 1))} disabled={page === pages} className="px-3 py-1 rounded-md hover:bg-gray-100 disabled:opacity-30 transition-colors">Next →</button>
            </div>
          </div>
        )}
      </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <input value={editFaq.question} onChange={e => setEditFaq(f => f ? { ...f, question: e.target.value } : null)}
                className="w-full px-3 py-2 rounded-md text-sm text-gray-800 bg-gray-50 border border-gray-200 outline-none focus:border-gray-400" />
            </div>
            <div>
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button onClick={() => setEditModal(false)} className="px-4 py-2 rounded-md text-sm text-gray-500 hover:text-gray-800 hover:bg-gray-100 transition-colors">Cancel</button>
              <button onClick={handleEdit} disabled={saving} className="px-4 py-2 rounded-md text-sm font-medium text-white bg-gray-900 hover:bg-gray-700 disabled:opacity-40 transition-colors">{saving ? 'Saving…' : 'Save'}</button>
            </div>
          </div>
        )}
      </Modal>
        ...
        <div className="space-y-3">
          <div>
            ...
            <input value={newFaq.question} onChange={e => setNewFaq(f => ({ ...f, question: e.target.value }))} placeholder="Enter the question…"
              className="w-full px-3 py-2 rounded-md text-sm text-gray-800 placeholder-gray-400 bg-gray-50 border border-gray-200 outline-none focus:border-gray-400" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Answer</label>
            <textarea rows={4} value={newFaq.answer} onChange={e => setNewFaq(f => ({ ...f, answer: e.target.value }))} placeholder="Enter the answer…"
              className="w-full px-3 py-2 rounded-md text-sm text-gray-800 placeholder-gray-400 bg-gray-50 border border-gray-200 outline-none focus:border-gray-400 resize-none" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Category</label>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={() => setAddModal(false)} className="px-4 py-2 rounded-md text-sm text-gray-500 hover:text-gray-800 hover:bg-gray-100 transition-colors">Cancel</button>
            <button onClick={handleAdd} disabled={saving || !newFaq.question || !newFaq.answer || !newFaq.category}
              className="px-4 py-2 rounded-md text-sm font-medium text-white bg-gray-900 hover:bg-gray-700 disabled:opacity-40 transition-colors">{saving ? 'Creating…' : 'Create FAQ'}</button>
          </div>
        </div>
      </Modal>
    </div>
